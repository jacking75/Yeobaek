...
public sealed class PopupHandler(TabManager tabs, BrowserTab opener)
{
    ...
    private async void OnNewWindowRequested(object? sender, CoreWebView2NewWindowRequestedEventArgs e)
    {
        ...
        {
            AppLog.Error(ex, "새 창 요청을 탭으로 열기");
            if (tab is not null) tabs.Close(tab);
            tabs.Notify(Notice.Error("새 탭을 열지 못했습니다. 잠시 후 다시 시도해 주세요."));
        }
        finally
        {
    ...
    private void NotifyBlockedPopup(string url)
    {
        ...
        tabs.Notify(Notice.Info($"팝업을 막았습니다: {host}", "열기", () => tabs.OpenTab(url, activate: true)));
    }
}
